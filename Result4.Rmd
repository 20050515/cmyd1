...
library(xgboost)       # 用于XGBoost模型
library(pROC)          # 用于ROC曲线计算
library(caret)         # 用于交叉验证和数据拆分
...
run_cross_validation <- function(data, selected_species, meta_data) {
  ...
  for (i in seq_along(folds)) {
    ...
    # 训练模型
    dtrain <- xgb.DMatrix(data = t(data_filtered[, train_indices]), label = as.numeric(meta_filtered$Cluster[train_indices]) - 1)
    dtest <- xgb.DMatrix(data = t(data_filtered[, test_indices]))
    ...
    # 计算ROC曲线数据
    roc_result <- roc(meta_filtered$Cluster[test_indices], predictions)
    roc_data_combined[[i]] <- data.frame(
      specificity = roc_result$specificities,
      sensitivity = roc_result$sensitivities,
      fold = i  # 添加折数
...
ggplot_obj <- ggplot(roc_data_combined_all, aes(x = 1 - specificity, y = sensitivity, color = model)) +  
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray") +  
  labs(x = "1 - Specificity (False Positive Rate)", y = "Sensitivity (True Positive Rate)", 
       title = "Average ROC Curves for XGBoost Models with 10-Fold Cross-Validation") +  
  theme_minimal() +  
  theme(panel.grid.major = element_line(color = "lightgray"),   
        panel.grid.minor = element_line(color = "lightgray"),   
        panel.border = element_rect(color = "black", fill = NA),  
...
ggsave(filename = file.path(output_dir, "roc_xgboost_check.tiff"),   
       plot = ggplot_obj, width = 7, height = 5, units = "in", dpi = 300)  
```
##svm
```{r pressure, echo=FALSE}
